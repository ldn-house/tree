...
- `version.py` - Git commit info, auto-generated by CI (or "dev" placeholder locally)
...
|----------|--------|-------------|
| `/` | GET | Status: hostname, IP, uptime, animation, MQTT, version info |
...
This downloads the latest firmware ZIP from [nightly.link](https://nightly.link), extracts it, writes the files, and reboots. The update includes: main.py, ota.py, mqtt.py, selfupdate.py, github.py, version.py.

The `version.py` file is auto-generated by CI with git commit info, which is exposed via the status API (`/`) and MQTT attributes (`firmware_version`, `firmware_branch`, `firmware_build_time`).
